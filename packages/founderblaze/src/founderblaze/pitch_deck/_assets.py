"""Shared asset helpers for pitch-deck providers."""
...
def assert_slide_count(count: int, *, where: str) -> None:
    if count < MIN_SLIDES or count > MAX_SLIDES:
        raise RuntimeError(
            f"{where}: pitch deck must have {MIN_SLIDES}-{MAX_SLIDES} pages, got {count}"
        )


def step_delay_seconds() -> float:
    raw = os.environ.get("PITCHDECK_STEP_DELAY_MS") or os.environ.get(
        "APPKIT_STEP_DELAY_MS"
    ) or os.environ.get("BRANDKIT_STEP_DELAY_MS", "3000")
    try:
...
def find_input_json(inputs: list[Any], kind: str) -> dict[str, Any] | None:
    for asset in inputs or []:
        meta = dict(getattr(asset, "metadata", None) or {})
        mt = getattr(asset, "media_type", "") or ""
        if mt == "application/json" or meta.get("kind") == kind:
            data = asset_json(asset)
            if meta.get("kind") == kind or kind in data or data:
                if meta.get("kind") == kind:
                    return data
                if kind == "pitch_plan" and "slides" in data:
                    return data
                if kind == "pitch_product_brief" and "product_name" in data:
                    return data
                if kind == "pitch_design" and (
                    "palette" in data or "typography" in data
                ):
                    return data
    for asset in inputs or []:
        meta = dict(getattr(asset, "metadata", None) or {})
